# -*- coding: utf-8 -*-
"""
Created on Fri Mar 25 19:32:37 2022

@author: Simen

Bipolar cell options 
"""
...
ops['fs'] = 1 # sampling rate of recording, determines binning for cell detection
...
ops["smooth_sigma"] = 1.15 # (float, default: 1.15) standard deviation in pixels of the gaussian used to smooth the phase correlation between the reference image and the frame which is being registered. A value of >4 is recommended for one-photon recordings (with a 512x512 pixel FOV).
...
ops["keep_movie_raw"] = False # (bool, default: False) whether or not to keep the binary file of the non-registered frames. You can view the registered and non-registered binaries together in the GUI in the “View registered binaries” view if you set this to True.
ops["maxregshift"] = .25
ops["th_badframes"] = 0.5
ops["keep_movie_raw"] = True
ops["two_step_registration"] = False # (int, default: 0) if 1,  run registration twice (for low SNR data). keep_movie_raw must be True for this to work.
ops["reg_tif"] = True
...
ops["threshold_scaling"] = 2 #(int, default: 5) This is the most relevant SNR-parameter. Controls threshold for detecting ROIs (how much they stand out from noise). Higher = less ROIs
ops["max_overlap"] = 0.5 # (float, default: 0.75) ROIs with more than ops[‘max_overlap’] fraction of their pixels overlapping with other ROIs will be discarded. Therefore, to throw out NO ROIs, set this to 1.0.
ops["high_pass"] = 10 #(int, default: 100) running mean subtraction across time with window of size ‘high_pass’. Values of less than 10 are recommended for 1P data where there are often large full-field changes in brightness
...
ops["connected"] = 0 
ops["min_neuropil_pixels"] = 0 # (int, default: 350) minimum number of pixels used to compute neuropil for each cell
...
np.save(r"C:\Users\SimenLab\OneDrive\Universitet\PhD\Suite2p\Ops\{}".format(ops_name), ops)
